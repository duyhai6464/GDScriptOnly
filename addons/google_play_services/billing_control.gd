...
signal purchase_consumed_error(returnDict: Dictionary)
signal purchase_consumed(returnDict: Dictionary)
...
func process_purchase(purchase):
	for product in purchase["products"]:
		if (product in ITEM_ACKNOWLEDGED) or (product in SUBSCRIPTIONS):
			...
			match product:
				ITEM_CONSUMATED[0]:
					SignalBus.buy_diamond.emit(40, purchase["original_json"])
				ITEM_CONSUMATED[1]:
					SignalBus.buy_diamond.emit(100, purchase["original_json"])
				ITEM_CONSUMATED[2]:
					SignalBus.buy_diamond.emit(250, purchase["original_json"])
		else:
...
func _on_purchase_consumed_error(error) -> void:
	print(JSON.stringify(error, "  "))
	purchase_consumed_error.emit(error)
